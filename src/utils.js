...
export const loadSettingsFromFile = async() => {
  try {
    await fs.readFile('settings.json', { encoding: 'utf-8' });
  } catch(e) {
    console.log(`Encountered error ${e} when trying to get the settings file.`);
    return false;
  }
}
...
export const generateStructure = async(settings) => {
...
export const uploadSave = async(settings, selectedGame, file, update) => {
...
export const updateManifest = async(settings, selectedGame) => {
  const file = {
    name: 'manifest.json',
...
export const downloadSave = async(settings, selectedGame) => {
  try {
...
export const createBackup = async(gamePaths) => {
...
export const addNewGame = async(settings, reader, cb) => {
